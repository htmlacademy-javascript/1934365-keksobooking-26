...
const createAccommodationType = (accommodationName, addTemplateElement) => {
  const typeArray = Object.keys(accommodationType);
  if (typeArray.includes(accommodationName)) {
...
const createFeatures = (featuresName, addTemplateElement) => {
  if (featuresName) {
  // Очищаем разметку блока features от элементов
    addTemplateElement.querySelector('.popup__features').textContent = '';
...
const createDescription = (descriptionName, addTemplateElement) => {
...
similarAdds.forEach((card) => {
  ...
  createAccommodationType(card.offer.type, cardElement);
  ...
  createDescription(card.offer.description, cardElement);
